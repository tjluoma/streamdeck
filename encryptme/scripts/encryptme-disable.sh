#!/usr/bin/env zsh -f
# Purpose: disable EncryptMe (it can be just running, but NOT actively securing connection)
#
...
if [[ "$EXIT" = "1" ]]
then
	msg "EncryptMe is not running, so obviously not securing the connection."
	exit 0
fi
...
if [[ "$STATUS" == "false" ]]
then
	msg "EncryptMe is running, but was not securing the connection."
	exit 0
fi
...
then
	...
	osascript -e 'tell application "System Events" to keystroke "e" using {command down, shift down, option down, control down}'
		# give it a few seconds to de-activate
	sleep 2
else
...
if [[ "$STATUS" == "false" ]]
then
	msg "EncryptMe is running but not securing the connection."
	exit 0
fi

die "Failed to disable EncryptMe"
